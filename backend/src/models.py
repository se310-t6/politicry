...
class Users(db.Model):
    ...
    def __init__(self, password: str, username: str = None, salt: bytes = None) -> None:
        self.username = username or "".join(
            random.choices(string.ascii_letters + string.digits, k=32)
        )
        self.salt = salt or uuid4().hex
        self.password_hashed = hashlib.sha512(
            (password + self.salt).encode("utf-8")
        ).hexdigest()
        self.date_created = datetime.utcnow()

    def is_password_correct(self, password: str) -> bool:
        password = hashlib.sha512((password + self.salt).encode("utf-8")).hexdigest()
        return password == self.password_hashed
...
class Detections(db.Model):
    ...
    def __init__(
        self, user_id: int, post_id: int, word: str, date_created=None
    ) -> None:
